...
unsafe impl Sync for BcmHost{}

impl BcmHost {
    pub fn get()->&'static mut Self{
        static mut HOST:Option<BcmHost> = None;

        if let Some(host) = unsafe{&mut HOST} {
            return host;
        }
        ...
        unsafe{
            HOST = Some(BcmHost { ptr: bcm2835, mem_fd });
            return HOST.as_mut().unwrap();
        }
    }
    ...
    pub fn get_mem_fd(&self)->libc::c_int{
        self.mem_fd
    }
}
