use super::{Mode, GpioPull, CORE_FREQ, utils::{MmioReg32, compile_time_size_assert, memory_barrier, get_static_peripheral}, PERIPHERALS, IoGpioPin};

const UART_RX_PIN_BCM: u8 = 15;
const UART_TX_PIN_BCM: u8 = 14;
const AUX_BASE_OFFSET:usize = 0x21_5000;
const AUX_MINI_UART_OFFSET:usize = AUX_BASE_OFFSET + 0x40;
...
impl MiniUart{
    pub(super) fn new(baudrate:u32)->Self{
        let gpio = unsafe{PERIPHERALS.get_gpio()};
        // alt5 is the uart1 which the mini uart uses
        let mut rx_pin = gpio.take_pin(UART_RX_PIN_BCM).into_io(Mode::Alt5);
        let mut tx_pin = gpio.take_pin(UART_TX_PIN_BCM).into_io(Mode::Alt5);
        ...
        let control_regs: &'static mut AuxControlRegisters = get_static_peripheral(AUX_BASE_OFFSET);
        let mini_uart_regs: &'static mut MiniUartRegisters = get_static_peripheral(AUX_MINI_UART_OFFSET);
        ...
        return Self{registers:mini_uart_regs, _aux_control_registers:control_regs, _pins:[rx_pin, tx_pin]};
    }
