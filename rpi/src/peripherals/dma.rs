...
use super::{utils::{MmioReg32, compile_time_size_assert, memory_barrier, get_static_peripheral}, gpu::GpuMemory, spi::SPI_BUFFER_SIZE};
...
impl DmaSpiTransferer{
    ...
    pub fn new(spi_enable_dma_flag:u32)->Self{
        let tx_registers:&'static mut DmaRegistersAccess = get_static_peripheral(Self::BCM_DMA0_OFFSET + (Self::TX_CHANNEL_NUMBER as usize * Self::DMA_CHANNEL_REGISTERS_SIZE));
        let rx_registers:&'static mut DmaRegistersAccess = get_static_peripheral(Self::BCM_DMA0_OFFSET + (Self::RX_CHANNEL_NUMBER as usize * Self::DMA_CHANNEL_REGISTERS_SIZE));
        ...
        let dma_enable_register:&mut MmioReg32 = get_static_peripheral(Self::BCM_DMA_ENABLE_REGISTER_OFFSET);
    ...
    // This function initialize the DMA control blocks in order to create what I call a DMA Blocks Chain (lol)
    // This chain can be triggered by writing the start address of the first block in the chain to the DMA control block register address 
    // The way the chains are constructed is - one standalone link that trasnfers a chunk of data (smaller than the DMA max transfer size)
    // to the SPI and a Chain that reads the output of the SPI and discards it, once the SPI finishes transfering data back to us 
    // it means that the first link has already finished transfering data to the SPI. The chain will trigger another block that setups
    // the next standalone link by writing to the DMA block address to the TX_DMA registers and starting it (This needs a chain of 3 links).
    // The last link that triggers the nect chunk transferring will continue the chain to the link that will read the SPI output and discard it.
    // This way Im constructing a chain long enough to transfer all the data even though its larger than the DMA max transfer size and triggering 
    // this chain by only coping the data to the DMA buffer and writing to the RX DMA and TX DMA start addresses.
