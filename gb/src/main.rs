...
use crate::{mbc_handler::*, multi_device_audio::*, sdl_joypad_provider::*};
use lib_gb::{
    apu::audio_device::*,
...
fn main() {
    ...
    let (_window, renderer, texture): (*mut SDL_Window, *mut SDL_Renderer, *mut SDL_Texture) = unsafe {
        ...
        let tex: *mut SDL_Texture = SDL_CreateTexture(
            rend,
    ...
    if check_for_terminal_feature_flag(&args, "--file-audio") {
        let wav_ad =
            wav_file_audio_device::WavfileAudioDevice::new(44100, GB_FREQUENCY, "output.wav");
    ...
    unsafe {
        ...
        loop {
            ...
            let scaled_buffer = extend_vec(
                frame_buffer,
                screen_scale as usize,
                SCREEN_WIDTH,
                SCREEN_HEIGHT,
            );
