...
use crate::audio::*;
use common::{emulation_menu::*, joypad_menu::*, mbc_handler::*, mpmc_gfx_device::*};
...
fn main() {
    ...
    while !(EMULATOR_STATE.exit.load(std::sync::atomic::Ordering::Relaxed)){
        ...
        let program_name = if check_for_terminal_feature_flag(&args, "--rom-menu"){
            ...
            common::get_rom_selection(roms_path.as_str(), menu_renderer, &mut provider)
        }
        else{
            args[1].clone()
        };
