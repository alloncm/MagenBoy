use crate::{peripherals::{Mode, dma::DmaSpiTransferer, utils::memory_barrier, PERIPHERALS}, drivers, configuration::peripherals::*};

use super::{utils::{MmioReg32, compile_time_size_assert}, GpioPin, PERIPHERALS_BASE_ADDRESS};

const SPI0_BASE_ADDRESS:usize = PERIPHERALS_BASE_ADDRESS + 0x20_4000;
...
impl Spi0{
    ...
    pub(super) fn new (dc_pin:u8)->Self{
        ...
        let mut spi_cs0 = gpio.take_pin(Self::SPI0_CE0_N_BCM_PIN, Mode::Output);
        let dc_pin = gpio.take_pin(dc_pin, Mode::Output);
        let spi_pins = [
            gpio.take_pin(Self::SPI0_MOSI_BCM_PIN, Mode::Alt0), 
            gpio.take_pin(Self::SPI0_SCLK_BCM_PIN, Mode::Alt0)
        ];

        let spi_registers = unsafe{&mut *(SPI0_BASE_ADDRESS as *mut SpiRegisters)};
