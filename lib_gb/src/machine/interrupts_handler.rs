...
use crate::utils::{bit_masks::*, memory_registers::IF_REGISTER_ADDRESS};
...
impl InterruptsHandler {
    pub fn handle_interrupts(
        ...
        if cpu.mie && self.ei_triggered {
            if interupt_flag & BIT_0_MASK != 0 && interupt_enable & BIT_0_MASK != 0 {
                return Self::prepare_for_interut(
                    cpu,
                    BIT_0_MASK,
                    V_BLANK_INTERRUPT_ADDERESS,
                    memory,
                    &mut interupt_flag,
                );
            }
            if interupt_flag & BIT_1_MASK != 0
                && interupt_enable & BIT_1_MASK != 0
                && (ppu.v_blank_interrupt_request
                    || ppu.oam_search_interrupt_request
                    || ppu.h_blank_interrupt_request
                    || ppu.coincidence_interrupt_request)
            {
                return Self::prepare_for_interut(
                    cpu,
                    BIT_1_MASK,
                    LCD_STAT_INTERRUPT_ADDERESS,
                    memory,
                    &mut interupt_flag,
                );
            }
    ...
    fn prepare_for_interut(
        cpu: &mut GbCpu,
        interupt_bit: u8,
        address: u16,
        memory: &mut impl Memory,
        interupt_flag: &mut u8,
    ) -> u8 {
