use crate::{
    mmu::{gb_mmu::GbMmu, io_ports::*, memory::UnprotectedMemory},
    utils::{bit_masks::*, memory_registers::*},
};

use super::{
    audio_device::AudioDevice, channel::Channel, frame_sequencer::FrameSequencer,
    freq_sweep::FreqSweep, gb_apu::GbApu, noise_sample_producer::NoiseSampleProducer,
    sample_producer::SampleProducer, sound_utils::NUMBER_OF_CHANNELS,
    square_sample_producer::SquareSampleProducer, volume_envelop::VolumeEnvlope,
...
fn prepare_tone_channel(
    channel: &mut Channel<SquareSampleProducer>,
    memory: &mut GbMmu,
    fs: &FrameSequencer,
) {
    if memory.io_ports.get_ports_cycle_trigger()[NR21_REGISTER_INDEX as usize] {
        channel.sound_length =
            64 - (memory.read_unprotected(NR21_REGISTER_ADDRESS) & 0b11_1111) as u16;
    }
    if memory.io_ports.get_ports_cycle_trigger()[NR22_REGISTER_INDEX as usize] {
        update_volume_envelope(
            memory.read_unprotected(NR22_REGISTER_ADDRESS),
            &mut channel.sample_producer.envelop,
        );

        if !is_dac_enabled(
            channel.sample_producer.envelop.volume,
            channel.sample_producer.envelop.increase_envelope,
        ) {
    ...
    if memory.io_ports.get_ports_cycle_trigger()[NR24_REGISTER_INDEX as usize] {
        ...
        let dac_enabled = is_dac_enabled(
            channel.sample_producer.envelop.volume,
            channel.sample_producer.envelop.increase_envelope,
        );
        ...
        if nr24 & BIT_7_MASK != 0 {
            //volume
            channel.sample_producer.envelop.envelop_duration_counter =
                channel.sample_producer.envelop.number_of_envelope_sweep;
            channel.sample_producer.envelop.current_volume = channel.sample_producer.envelop.volume;
        }
    }
}

fn prepare_noise_channel(
    channel: &mut Channel<NoiseSampleProducer>,
    memory: &mut GbMmu,
    fs: &FrameSequencer,
) {
    ...
    if memory.io_ports.get_ports_cycle_trigger()[NR42_REGISTER_INDEX as usize] {
        update_volume_envelope(
            memory.read_unprotected(NR42_REGISTER_ADDRESS),
            &mut channel.sample_producer.envelop,
        );
        if !is_dac_enabled(
            channel.sample_producer.envelop.volume,
...
fn prepare_control_registers<AD: AudioDevice>(
    apu: &mut GbApu<AD>,
    memory: &impl UnprotectedMemory,
) {
...
fn prepare_wave_channel(
    channel: &mut Channel<WaveSampleProducer>,
    memory: &mut GbMmu,
    fs: &FrameSequencer,
) {
...
fn update_channel_conrol_register<T: SampleProducer>(
    channel: &mut Channel<T>,
    dac_enabled: bool,
    control_register: u8,
    max_sound_length: u16,
    fs: &FrameSequencer,
) {
    ...
    let possible_extra_length_clocking =
        !previous_length_enable && channel.length_enable && channel.sound_length != 0;
    ...
    if (control_register & BIT_7_MASK) != 0 {
        ...
        channel.timer.update_cycles_to_tick(
            channel
                .sample_producer
                .get_updated_frequency_ticks(channel.frequency),
        );
    }
}
