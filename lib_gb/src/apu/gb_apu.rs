use super::{
    audio_device::*, channel::Channel, frame_sequencer::*, freq_sweep::FreqSweep,
    noise_sample_producer::NoiseSampleProducer, sound_terminal::SoundTerminal,
    sound_utils::NUMBER_OF_CHANNELS, square_sample_producer::SquareSampleProducer,
    wave_sample_producer::WaveSampleProducer,
};
use crate::{
    mmu::memory::UnprotectedMemory,
    utils::{
        bit_masks::set_bit_u8,
        memory_registers::{NR10_REGISTER_ADDRESS, NR52_REGISTER_ADDRESS},
    },
};
...
impl<Device: AudioDevice> GbApu<Device> {
    pub fn new(device: Device) -> Self {
        GbApu {
            ...
            sweep_tone_channel: Channel::<SquareSampleProducer>::new(
                SquareSampleProducer::new_with_sweep(),
            ),
    ...
    fn update_channels_for_frame_squencer(&mut self, tick: TickType) {
        if tick.frequency_sweep {
            if self.sweep_tone_channel.enabled {
                let sweep: &mut FreqSweep = &mut self
                    .sweep_tone_channel
                    .sample_producer
                    .sweep
                    .as_mut()
    ...
    fn update_registers(&mut self, memory: &mut impl UnprotectedMemory) {
        ...
        set_bit_u8(
            &mut control_register,
            3,
            self.noise_channel.enabled
                && self.noise_channel.length_enable
                && self.noise_channel.sound_length != 0,
        );
        set_bit_u8(
            &mut control_register,
            2,
            self.wave_channel.enabled
                && self.wave_channel.length_enable
                && self.wave_channel.sound_length != 0,
        );
        set_bit_u8(
            &mut control_register,
            1,
            self.tone_channel.enabled
                && self.tone_channel.length_enable
                && self.tone_channel.sound_length != 0,
        );
        set_bit_u8(
            &mut control_register,
            0,
            self.sweep_tone_channel.enabled
                && self.sweep_tone_channel.length_enable
                && self.sweep_tone_channel.sound_length != 0,
        );
