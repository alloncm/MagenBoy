    ...
    fn read(&self, address: u16) -> u8 {
        if let Some(bus) = &self.dma_state {
            return match address {
                ...
                0x8000..=0x9FFF => {
                    if let AccessBus::External = bus {
                        self.read_unprotected(address)
                    } else {
                        Self::bad_dma_read(address)
                    }
                }
                0..=0x7FFF | 0xA000..=0xFDFF => {
                    if let AccessBus::Video = bus {
                        self.read_unprotected(address)
                    } else {
                        Self::bad_dma_read(address)
                    }
    ...
    fn write(&mut self, address: u16, value: u8) {
        if let Some(bus) = &self.dma_state {
            match address {
                ...
                0x8000..=0x9FFF => {
                    if let AccessBus::External = bus {
                        self.write_unprotected(address, value)
                    } else {
                        Self::bad_dma_write(address)
                    }
                }
                0..=0x7FFF | 0xA000..=0xFDFF => {
                    if let AccessBus::Video = bus {
...
impl<'a> GbMmu<'a> {
    ...
    pub fn new(mbc: &'a mut Box<dyn Mbc>) -> Self {
        ...
        mmu.io_ports
            .write_unprotected(BOOT_REGISTER_ADDRESS - 0xFF00, 1);
