...
pub fn update_mmu_registers(memory: &mut GbMmu, dma:&mut OamDmaTransferer, ports:&[bool;IO_PORTS_SIZE]){
    ...
    if ports[0x46]{
        handle_dma_transfer_register(memory, memory.read_unprotected(DMA_REGISTER_ADDRESS), dma);
    }
}
...
fn handle_dma_transfer_register(memory: &mut GbMmu, register:u8, dma: &mut OamDmaTransferer){
