...
const BLANK_SCREEN_BUFFER: [u32; SCREEN_HEIGHT * SCREEN_WIDTH] =
    [GbPpu::color_as_uint(&WHITE); SCREEN_HEIGHT * SCREEN_WIDTH];
...
impl GbPpu {
            ...
            if !self.line_rendered {
                ...
                for i in line_index..line_index + SCREEN_WIDTH {
                    self.screen_buffer[i] =
                        Self::color_as_uint(&frame_buffer_line[(i - line_index)]);
                }
            }
        }
    }
    ...
    fn get_bg_frame_buffer(&self, memory: &impl UnprotectedMemory) -> [Color; SCREEN_WIDTH] {
        ...
        let index =
            ((current_line.wrapping_add(self.background_scroll.y)) / NORMAL_SPRITE_HIEGHT) as u16;
        if self.window_tile_background_map_data_address {
            ...
            for i in 0..BG_SPRITES_PER_LINE {
                let mut chr: u8 =
                    memory.read_unprotected(address + (index * BG_SPRITES_PER_LINE) + i);
        ...
        for i in 0..line_sprites.len() {
            for j in 0..SPRITE_WIDTH {
                let pixel =
                    line_sprites[i].pixels[((sprite_line as u8 * SPRITE_WIDTH) + j) as usize];
    ...
    fn draw_window_frame_buffer(
        &mut self,
        memory: &impl UnprotectedMemory,
        line: &mut [Color; SCREEN_WIDTH],
    ) {
        if !self.window_enable
            || !self.background_enabled
            || self.current_line_drawn < self.window_scroll.y
        {
            return;
        }
    ...
    fn draw_objects_frame_buffer(
        ...
        for obj_attribute in &obj_attributes {
            let mut sprite = Self::get_sprite(
                obj_attribute.tile_number,
                memory,
                0x8000,
                self.sprite_extended,
            );
    ...
    fn get_line(
        ...
        for k in (0..SPRITE_WIDTH).rev() {
            ...
            unsafe {
                (*sprite).set_pixel(line_number * SPRITE_WIDTH + swaped, value);
            }
        }
    }
}
