...
use crate::{
    cpu::gb_cpu::GbCpu,
    utils::memory_registers::{IE_REGISTER_ADDRESS, JOYP_REGISTER_ADDRESS},
};
...
pub fn stop(cpu: &mut GbCpu, memory: &mut impl Memory) -> u8 {
    if (memory.read(IE_REGISTER_ADDRESS) & 0b11111 == 0)
        && (memory.read(JOYP_REGISTER_ADDRESS) & 0b1111 == 0)
    {
        cpu.stop = true;
    }
